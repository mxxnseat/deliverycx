...
class Iiko {
    ...
    private async getOrganizations() {
        try {
            ...
            const organizationsResponse = await axios.get<IOrganizationResponse[]>(`https://iiko.biz:9900/api/0/organization/list?access_token=${Iiko.token}`);
        ...
        try {
            ...
            for (let k in this.organizations) {
                ...
                await Promise.all(nomenclature.categories.map(async (category) => {
                    await CategoryModel.findOneAndUpdate({ _id: category.id }, {
                        ...category,
                        _id: category.id
                    }, { upsert: true });
                }));
